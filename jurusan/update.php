...
if (isset($_GET['id'])) {
    ...
    $q = $conn->query("SELECT * FROM jurusan WHERE id_jurusan = '$id'");
    $dt = $q->fetch_assoc();
?>
    <!DOCTYPE html>
    <html lang="en">
    ...
    <head>
        ...
        <script>
            function validateForm() {
                const namaJurusan = document.getElementById("nama").value;
                if (namaJurusan.trim() === "") {
                    alert("Nama Jurusan harus diisi");
                    return false;
                }
                return confirm("Apakah anda yakin untuk mengedit data jurusan?");
            }
        </script>
    </head>
    ...
    <body>
        ...
        <ul class="nav nav-pills nav-fill">
            ...
            <h2 class="text-center mt-5">Ubah Data Jurusan</h2>
            <div class="m-5 px-5">
                <legend>Data Jurusan Baru</legend>
                <form action="proses_update.php?id=<?= $id ?>" method="post" onsubmit="return validateForm();" class="row g-3">
                    <div class="col-12">
                        <label for="nama" class="form-label">Nama Jurusan:</label>
                        <input type="text" name="nama" id="nama" class="form-control" value="<?= htmlspecialchars($dt['nama_jurusan'], ENT_QUOTES, 'UTF-8') ?>">
                    </div>
                    <div class="col-12">
                        <input type="submit" class="btn btn-primary" name="submit" value="Ubah">
                        <button type="button" class="btn"><a href="data_jurusan.php">Kembali</a></button>
                    </div>
                </form>
            </div>
        </div>
